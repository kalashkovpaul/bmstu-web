 ...
 */
  ...
  addCreateBookmarkButtonListener = () => {
    ...
    createBookmarkButton.addEventListener('click', (e) => {
      e.preventDefault();
      const name = document.querySelector('.bookmark-name') as HTMLInputElement;
      const surname = document.querySelector('.bookmark-surname') as HTMLInputElement;
      const patronimic = document.querySelector('.bookmark-patronimic') as HTMLInputElement;
      const height = document.querySelector('.bookmark-height') as HTMLInputElement;
      const weight = document.querySelector('.bookmark-weight') as HTMLInputElement;
      const room = document.querySelector('.bookmark-room') as HTMLInputElement;
      const danger = document.querySelector('.bookmark-danger') as HTMLInputElement;

      if (name && surname && patronimic && height && weight && room && danger) {
        this.createBookmark({
          Name: name.value,
          Surname: surname.value,
          Patronymic: patronimic.value,
          Height: +height.value,
          Weight: +weight.value,
          RoomNumber: +room.value,
          DegreeOfDanger: +danger.value,
        });
  ...
  createBookmark = (data: bookmarkCreateRequest) => {
    this.eventBus.emit(events.profilePage.createBookmark, data, this.userData.ID);
  };
